...
breed [ units unit ]
units-own [ grph indx membr filled? ]
...
to setup-histograms 
  ...
  set goal-list-a (list (10 * root-a) (10 * tuber-a) (10 * leaf-a) (10 * fruit-a)) 
  set goal-list-b (list (10 * root-b) (10 * tuber-b) (10 * leaf-b) (10 * fruit-b))
  
end

to-report randomize [ base-value ]
  let half .5 * base-value
  report half + random-normal half (.2 * base-value)
end
...
to setup
  ...
  create-a-random-plant  -.25  .25
  create-a-random-plant  .25  .25
  create-a-random-plant  .25  -.25
  create-a-random-plant  -.25  -.25
 
  setup-graph-for "a" min-pxcor - .2 max-pycor - .2 goal-list-a
  setup-graph-for "b" max-pxcor - .2 max-pycor - .2 goal-list-b
...
to run-turn
  ...
  ask plants [
   ...
   ifelse (availables < 0) 
   [ set extras draw-down-resources-with-remainder availables ]
   [ set extras spend-resources-with-remainder availables ]
   
   if (fruit-of part-states >= 2) [ 
     set part-states replace-item 3 part-states (item 3 goal-list - 2)
...
to launch-offspring 
  let p one-of pot-neighbors with [ count plants-here < 4 ]
  if p != nobody
  [
    let coords (take-free-spot-from p)
    let xc item 0 coords
...
to-report take-free-spot-from [apatch]
  ;;need to keep a record.
  let rx one-of [-.25 .25]
  let ry one-of [-.25 .25]
  report (list ([pxcor] of apatch + rx) ([pycor] of apatch + ry))
end
...
to-report take-toll-from [ resource-units ]
  let toll-factor .15
  let count-costly-parts root-of part-states + leaf-of part-states + fruit-of part-states ;everything except tubers
;  show (word "toll should be = " ((toll-factor) * count-costly-parts) ) 
  report resource-units - ((toll-factor) * count-costly-parts)
end

to-report spend-resources-with-remainder [resource-units]
  let usables floor resource-units
  set water-resources water-resources - usables
  set sun-resources sun-resources - usables
  repeat usables [
     let what-index prioritized-selection goal-list
     ...
     ;fill-one grph what-index ;;not graphing 
  ]
  report resource-units - usables
end


to-report draw-down-resources-with-remainder [ resource-debt ]
  let debt -1 * resource-debt
  let leftovers 0
  let covered tuber-of part-states
  ifelse covered >= ceiling debt
  [ 
    set leftovers (ceiling debt - debt)  
    set part-states replace-item 1 part-states (covered - ceiling debt)  
    report leftovers
  ]
  [
    set part-states replace-item 1 part-states 0 
    set debt debt - covered
    while [ debt > 0 ]
    [
       let ind prioritized-selection part-states
       set debt debt - 1
       set part-states replace-item ind part-states (item ind part-states - 1) 
    ]
    report -1 * debt
  ]
end
...
to-report stochastic-spend-on [glist]
  let i 0
  let selection-list []
  foreach glist [
   repeat ? [ set selection-list lput (i) selection-list ]
   set i i + 1
  ]
  report one-of selection-list
end

to-report prioritized-selection [glist]
  let i 0
  let to-select max glist
  let selectable-indices []
  foreach glist [
   if (? = to-select) [ set selectable-indices lput i selectable-indices ]
   set i i + 1
  ]
  report one-of selectable-indices
end

to test-prioritized-spend-on
  let good true
  if prioritized-selection [ 0 0 0 1 ] != 3 [user-message "error with [0 0 0 1]" set good false]
  if prioritized-selection [ 0 0 1 0 ] != 2 [user-message "error with [0 0 1 0]" set good false]
  if prioritized-selection [ 1 0 0 0 ] != 0 [user-message "error with [1 0 0 0]" set good false]
...
;;histograms
to setup-graph-for [graph xleft ybot alist]
  let index 0
  while [ index < length alist ]
  [
    let val item index alist
    let member 1
    while [member <= val ] 
    [
     create-units 1 [ 
       set heading 0 set color yellow set size .1 setxy (xleft + .15 * index) (ybot + member * .1) 
       set grph graph
       set indx index
       set membr member
       set filled? false
...
to fill-one [ graph index ]
  let available-column units with [ grph = graph and indx = index and filled? = false]
  ask max-one-of available-column [ membr ] [ set filled? true set color yellow - 3 ]
end
@#$#@#$#@
GRAPHICS-WINDOW
323
